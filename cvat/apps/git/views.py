...
def check_process(request, rq_id):
    try:
        ...
        slogger.glob.error("error occured during checking repository request with rq id {}".format(rq_id),
                           exc_info=True)
...
def create(request, tid):
    try:
        ...
        slogger.glob.error("error occured during initial cloning repository request with rq id {}".format(rq_id),
                           exc_info=True)
        return HttpResponseBadRequest(str(ex))
...
def push_repository(request, tid):
    try:
        ...
        queue = django_rq.get_queue('default')
        queue.enqueue_call(func=CVATGit.push, args=(tid, request.user, request.scheme, request.get_host()),
                           job_id=rq_id)
