...
class ArchiveReader(DirectoryReader):
    def __init__(self, source_path, step=1, start=0, stop=None):
        ...
        super().__init__(source_path=[extract_dir], step=step, start=start, stop=stop)
...
class ZipReader(ImageListReader):
    ...
    def extract(self):
        self._zip_source.extractall(
            self.extract_dir if self.extract_dir else os.path.dirname(self._zip_source.filename))
