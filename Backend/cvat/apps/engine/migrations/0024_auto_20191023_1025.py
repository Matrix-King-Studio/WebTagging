            ...
            if original_video:
                if os.path.exists(original_video):
                    ...
                    for chunk_idx, chunk_image_ids in enumerate(slice_by_size(range(size), chunk_size)):
                        ...
                        shutil.copyfile(get_frame_path(old_task_data_dir, image_id),
...
def create_data_objects(apps, schema_editor):
    ...
    for counter, db_task in enumerate(db_tasks.all().iterator()):
        ...
        log.info(
            'Schema migration for the task {} completed. Progress {}/{}'.format(db_task.id, counter + 1, task_count))
        if res:
    ...
    def create_process(db_task_id, db_data_id):
        ...
        if hasattr(db_data, 'video'):
            ...
            original_images = [os.path.realpath(os.path.join(new_raw_dir, db_image.path)) for db_image in
                               db_data.images.all()]

        args = (db_task_id, db_data_id, original_video, original_images, db_data.size,
                db_data.start_frame, db_data.stop_frame, db_data.frame_filter, db_data.image_quality,
                db_data.chunk_size, return_dict)
    ...
    while True:
            ...
            if not res.is_alive():
                ...
                if res.exitcode == 0:
                    ...
                    if ret_code:
                        ...
                        log.info('Data migration for the task {} completed. Progress: {:.02f}% | {}/{}.'.format(res_idx,
                                                                                                                progress,
                                                                                                                counter,
                                                                                                                task_count))
                    else:
        ...
        while task_idx < len(migrated_db_tasks) and len(results) < MIGRATION_THREAD_COUNT:
            log.info('Start data migration for the task {}, data ID {}'.format(migrated_db_tasks[task_idx][0],
                                                                               migrated_db_tasks[task_idx][1]))
        ...
        for db_model in DLModel.objects.all():
            try:
                ...
                if os.path.isdir(old_location):
                    ...
                    db_model.interpretation_file.name = db_model.interpretation_file.name.replace(old_location,
                                                                                                  new_location)
...
class Migration(migrations.Migration):
    ...
    operations = [
        migrations.CreateModel(
            ...
            fields=[
                ...
                ('compressed_chunk_type',
                 models.CharField(choices=[('video', 'VIDEO'), ('imageset', 'IMAGESET'), ('list', 'LIST')],
                                  default=DataChoice('imageset'), max_length=32)),
                ('original_chunk_type',
                 models.CharField(choices=[('video', 'VIDEO'), ('imageset', 'IMAGESET'), ('list', 'LIST')],
                                  default=DataChoice('imageset'), max_length=32)),
            ],
        ...
        migrations.AddField(
            model_name='video',
            name='data',
            field=models.OneToOneField(null=True, on_delete=django.db.models.deletion.CASCADE, related_name='video',
                                       to='engine.Data'),
        ),
        migrations.AddField(
            model_name='clientfile',
            name='data',
            field=models.ForeignKey(null=True, on_delete=django.db.models.deletion.CASCADE, related_name='client_files',
                                    to='engine.Data'),
        ),
        migrations.AddField(
            model_name='remotefile',
            name='data',
            field=models.ForeignKey(null=True, on_delete=django.db.models.deletion.CASCADE, related_name='remote_files',
                                    to='engine.Data'),
        ),
        migrations.AddField(
            model_name='serverfile',
            name='data',
            field=models.ForeignKey(null=True, on_delete=django.db.models.deletion.CASCADE, related_name='server_files',
                                    to='engine.Data'),
        ),
