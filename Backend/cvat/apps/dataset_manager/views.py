...
def log_exception(logger=None, exc_info=True):
    if logger is None:
        logger = slogger
    logger.exception("[%s @ %s]: exception occurred" % (_MODULE_NAME, current_function_name(2)), exc_info=exc_info)
...
def export_task(task_id, dst_format, server_url=None, save_images=False):
    try:
        ...
        output_base = '%s_%s' % ('dataset' if save_images else 'annotations', make_file_name(to_snake_case(dst_format)))
        ...
        if not (osp.exists(output_path) and task_time <= osp.getmtime(output_path)):
            ...
            with tempfile.TemporaryDirectory(dir=cache_dir) as temp_dir:
                ...
                task.export_task(task_id, temp_file, dst_format, server_url=server_url, save_images=save_images)
